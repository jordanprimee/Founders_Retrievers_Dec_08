      ...
      <Swiper {...swiperParams}>
        {Array.isArray(data.comments) &&
          data.comments.map((item) => (
            <SwiperSlide
              key={item.id_contact}
              className="lg:scale-100 scale-75 md:scale-90 sm:scale-90 "
            >
              <div className="flex flex-row justify-items-center ">
                <div className="flex flex-row gap-6  bg-[#86868650] p-4 rounded-[0.5rem] hover:scale-105">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[#000] font-light text-[0.9rem]">
                      {item.full_name}
                    </span>
                    <span className="text-[#00000095] font-light text-[0.75rem]  max-h-4 h-8 hover:h-12 max-w-8 w-48 overflow-hidden hover:overflow-visible ">
                      {item.message}
                    </span>
                  </div>
                </div>
